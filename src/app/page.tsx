...
export default function Home() {
  ...
  async function runPreview() {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        setError(json.ok ? { code: "UNKNOWN", message: "Preview failed" } : json.error);
        return;
      }
  ...
  async function runCommit() {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        setError(json.ok ? { code: "UNKNOWN", message: "Commit failed" } : json.error);
        return;
      }
  ...
  return (
    <div className="flex flex-col flex-1 items-center bg-zinc-50 px-6 py-12 font-sans dark:bg-black">
      <main className="flex w-full max-w-3xl flex-col gap-6 rounded-lg bg-white p-6 shadow-sm dark:bg-zinc-950">
        <div className="flex flex-col gap-1">
          <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-50">
            Screener Import (Preview)
          </h1>
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            Upload a Screener XLSX and parse only the &quot;Data Sheet&quot; tab.
          </p>
        </div>
